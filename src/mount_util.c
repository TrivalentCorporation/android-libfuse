...
{
	int isempty = 1;
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "top of fuse_mnt_check_empty");
	...
	if (S_ISDIR(rootmode)) {
		...
		if (dp == NULL) {
			fprintf(stderr,
				...
				progname, strerror(errno));
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "fuse_mnt_check_empty 1");
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "fuse_mnt_check_empty mnt:%s", mnt);
			return -1;
		}
	...
	if (!isempty) {
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "fuse_mnt_check_empty progname:%s", progname);
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "fuse_mnt_check_empty mnt:%s", mnt);
		fprintf(stderr, "%s: mountpoint is not empty\n", progname);
		fprintf(stderr, "%s: if you are sure this is safe, use the 'nonempty' mount option\n", progname);
__android_log_print(ANDROID_LOG_DEBUG, "fusedebug", "fuse_mnt_check_empty 2");
		return -1;
	}
	return 0;
}
